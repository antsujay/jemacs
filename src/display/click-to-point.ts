...
export function windowClickState(
  ...
  return { startLine: start, gutterPrefixLen: gutter }
}
...
export function pointFromWindowClick(
  ...
  const colInLine = Math.max(0, Math.min(col - state.gutterPrefixLen, line.length))
  const point = lineStart + colInLine
  const maxPoint = bufferText.length
