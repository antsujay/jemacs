...
describe("term-v2: install wiring", () => {
  test("registers term commands and char-mode keymap", () => {
    ...
    expect(editor.commands.get("term-interrupt")).toBeUndefined()
    expect(editor.commands.get("term-kill")).toBeUndefined()
    expect(editor.commands.get("term-interrupt-subjob")).toBeDefined()
    expect(editor.commands.get("term-kill-subjob")).toBeDefined()
    const map = getMode("term")?.keymap
    expect(map?.get("a")).toBe("term-send-raw")
    expect(map?.get("C-c C-c")).toBe("term-interrupt-subjob")
  })
})
